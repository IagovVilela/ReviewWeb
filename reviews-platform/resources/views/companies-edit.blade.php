...
<div class="max-w-4xl mx-auto pb-6 sm:pb-8">
    <form method="POST" action="{{ route('companies.update', $company->id) }}" enctype="multipart/form-data" id="companyForm" class="space-y-3 sm:space-y-6 pb-12 sm:pb-6" data-media-save-url="{{ route('companies.auto-save-media', $company->id, false) }}" data-media-save-token="{{ $mediaAutoSaveToken }}">
        ...
        <!-- Transferir empresa -->
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-3 sm:p-6 mt-4">
            <h2 class="text-lg sm:text-xl font-bold text-gray-800 dark:text-gray-100 mb-2">
                <i class="fas fa-exchange-alt mr-2 text-amber-600"></i>
                {{ __('companies.transfer_company') }}
            </h2>
            <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">{{ __('companies.transfer_company_desc') }}</p>
            <form action="{{ route('companies.transfer', $company->id) }}" method="POST" class="flex flex-col sm:flex-row gap-2" onsubmit="return confirm('{{ __('companies.transfer_company') }}? O novo usuário será o proprietário.');">
                @csrf
                @method('PUT')
                <select name="new_user_id" required class="flex-1 px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 text-sm">
                    <option value="">{{ __('companies.transfer_to') }}...</option>
                    @foreach($users as $u)
                        @if($u->id !== $company->user_id)
                            <option value="{{ $u->id }}">{{ $u->name }} ({{ $u->email }})</option>
                        @endif
                    @endforeach
                </select>
                <button type="submit" class="px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white rounded-lg text-sm font-medium inline-flex items-center gap-2">
                    <i class="fas fa-paper-plane"></i>
                    {{ __('companies.transfer_submit') }}
                </button>
            </form>
        </div>
        @endif
